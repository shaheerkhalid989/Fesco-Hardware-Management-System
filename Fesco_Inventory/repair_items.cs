...
{
    public partial class repair_items : UserControl
    {
        Panel pan;
        userBL user;
        Label label;
        public repair_items(Panel pan, userBL user, Label label)
        {
            InitializeComponent();
            this.pan = pan;
            this.user = user;
        ...
        void retrieve()
        {
            ...
            SqlCommand cmd = new SqlCommand("select r.Updated, r.id, r.FolioNo,(select ItemNo from Inventory where id = r.ItemId) ItemNO,(select Item_Description from Inventory where id = r.ItemId)ItemName,r.Quantity, (select Reason_Repair from Repair where id = r.Id)rRepair,(select [Value] from [Lookup] where id = r.Approved_By and Category = 'APPROVER')ApprovedBy,r.Issuing_officer,(select Office_Code from Office where id = r.OfficeId)OfficeId,(select office_Add from Office where id = r.OfficeId)OfficeName,r.Receiver,r.Receiver_Designation,r.Received_By,(select EmployeeName from Employee where id = r.Requisit_By)[Requisit_By],r.Requisition_Date,r.Receving_Date from Requisition r where r.Nature_of_Work = (select id from [Lookup] where [Value] = 'Item Repair' and Category = 'NATURE_OF_WORK')", con);
        ...
        {
            var con = Configuration.getInstance().getConnection();
            SqlCommand cmd = new SqlCommand(string.Concat("select r.Updated, r.id, r.FolioNo,(select ItemNo from Inventory where id = r.ItemId) ItemNO,(select Item_Description from Inventory where id = r.ItemId)ItemName,r.Quantity, (select Reason_Repair from Repair where id = r.Id)rRepair,(select [Value] from [Lookup] where id = r.Approved_By and Category = 'APPROVER')ApprovedBy,r.Issuing_officer,(select Office_Code from Office where id = r.OfficeId)OfficeId,(select office_Add from Office where id = r.OfficeId)OfficeName,r.Receiver,r.Receiver_Designation,r.Received_By,(select EmployeeName from Employee where id = r.Requisit_By)[Requisit_By],r.Requisition_Date,r.Receving_Date from Requisition r where r.FolioNo like '",textbox_search.Text, "%' and Year(r.Requisition_Date) = ",textbox_year.Text," and r.Nature_of_Work = (select id from [Lookup] where [Value] = 'Item Repair' and Category = 'NATURE_OF_WORK')"), con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            dataGridView1.DataSource = dt;
        ...
        private void repair_items_Load(object sender, EventArgs e)
        {
            label.Text = "Repairing Requisitions";
            retrieve();
        }
